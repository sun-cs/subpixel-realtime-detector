...
#include <opencv2/opencv.hpp>  //添加OpenCV引用
...
int CBasicDemoDlg::SaveImage(MV_SAVE_IAMGE_TYPE enSaveImageType)
{
    ...
    stSaveImageParam.nEndian = 0;       //保存TIFF图像时的字节序 1-大端存储  0、其他值-小端存储
...
int CBasicDemoDlg::GrabThreadProcess()
{
    ...
    while(m_bThreadState)
    {
        ...
        if (nRet == MV_OK)
        {
			...
			m_pcMyCamera->DisplayOneFrame((void*)m_hwndDisplay, &stImageData);
            ...
            {
                srcImage = cv::Mat(nHeight, nWidth, CV_8UC1, stImageInfo.pBufAddr);
            }
            else if (stImageInfo.stFrameInfo.enPixelType == PixelType_Gvsp_BGR8_Packed)
            {
                srcImage = cv::Mat(nHeight, nWidth, CV_8UC3, stImageInfo.pBufAddr);
            }

            if (!srcImage.empty())
            {
                // 显示原始图像窗口
                cv::imshow("1. 原图 (Original)", srcImage);
                ...
                {
                    grayImage = srcImage.clone(); // 已经是灰度图，直接拷贝
                }
                ...
                // 4. Canny 边缘检测算法
                cv::Mat edgeImage;
                cv::Canny(blurImage, edgeImage, 50, 150); // 50和150是双阈值

                // 显示边缘检测结果窗口
                cv::imshow("2. 边缘检测结果 (Canny Edges)", edgeImage);
...
void CBasicDemoDlg::OnBnClickedCircleAuxiliaryButton()
{
    ...
    stCircleInfo.stColor.fAlpha = 0.5;
